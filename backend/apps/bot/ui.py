...
def build_recommend_intro_text() -> str:
    return (
        "Здесь вы можете взять свою персональную ссылку\n"
        "и готовый текст для отправки клиенту."
    )


def build_spend_intro_text() -> str:
    return (
        "Здесь можно посмотреть подарки за бонусы,\n"
        "правила списания и идеи продукции."
    )


def build_help_intro_text() -> str:
    return (
        "Здесь собраны правила программы\n"
        "и связь с администратором."
    )
...
def build_member_start_text(*, full_name: str) -> str:
    return (
        ...
        "С возвращением! Здесь вы можете посмотреть свою ссылку, "
        "проверить бонусы, отследить заявки и выбрать подарок."
    )
