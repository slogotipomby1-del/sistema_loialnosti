...
HOW_IT_WORKS_BUTTON_TEXT = "🧭 Как это работает"
FAQ_BUTTON_TEXT = "❓ FAQ"
...
def build_help_menu_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text=HOW_IT_WORKS_BUTTON_TEXT)],
...
def build_help_intro_text() -> str:
    return (
        ...
        "Вы можете:\n"
        "— быстро понять, как работает программа;\n"
        "— быстро посмотреть правила;\n"
        "— открыть ответы на частые вопросы;\n"
        "— уточнить спорный вопрос;\n"
        "— написать администратору."
    )


def build_how_it_works_text() -> str:
    return (
        "Как работает программа «Мерч-бонусы»:\n\n"
        "1. Вы регистрируетесь в боте.\n"
        "2. Получаете персональную ссылку для рекомендации.\n"
        "3. Отправляете её знакомому, партнёру или компании, которой может быть нужен мерч.\n"
        "4. Новый клиент переходит по ссылке и оставляет заявку.\n"
        "5. После оплаты, отгрузки и ручной проверки вам начисляются бонусы.\n"
        "6. Бонусы можно использовать на подарок, доставку, нанесение или часть следующего заказа.\n\n"
        "Важно: рекомендация закрепляется по первой ссылке, а бонусы подтверждаются только после проверки заказа."
    )
...
def build_faq_text() -> str:
    return (
        "FAQ по программе:\n\n"
        "1. Когда начисляются бонусы?\n"
        "После оплаты, отгрузки и ручного подтверждения администратором.\n\n"
        "2. Можно ли вывести бонусы деньгами?\n"
        "Нет, бонусы можно использовать только внутри программы.\n\n"
        "3. На что можно потратить бонусы?\n"
        "На подарок, доставку, нанесение или часть следующего заказа.\n\n"
        "4. Что если клиент уже был в базе?\n"
        "Рекомендация засчитывается по правилам программы и проверяется вручную.\n\n"
        "5. Почему баланс может стать отрицательным?\n"
        "Если бонусы были использованы, а потом начисление пришлось аннулировать после возврата или отмены заказа."
    )
...
def build_balance_text(*, balance: str) -> str:
    ...
    if balance.startswith("-"):
        text += (
            "\n\n"
            "Сейчас баланс отрицательный. Это возможно, если бонусы по одному из заказов были аннулированы "
            "после возврата или отмены."
        )
    return text
...
def build_bonus_history_text(*, history_lines: list[str]) -> str:
    ...
    return (
        "История бонусов:\n"
        f"{history_block}\n\n"
        "Здесь показываются подтверждённые и зафиксированные операции:\n"
        "— начисления бонусов;\n"
